...
if [ $checkip == "0" ]; then
  if [ $checkowner == "0" ]; then
    ...
    echo "i was not the owner; seems i can start now, after stopping the other node" >> /bpm/scripts/reloc.log; 
    if [ $thishost == "node1" ]; then
      ssh root@node2 /bpm/scripts/stop_db.sh;
      ssh root@node2 /bpm/scripts/cleanup_db.sh;
    elif [ $thishost == "node2" ]; then
      ssh root@node1 /bpm/scripts/stop_db.sh;
